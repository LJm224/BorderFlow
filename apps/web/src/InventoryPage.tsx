...
interface Inventory { id: string; skuId: string; warehouseId: string; availableQuantity: number; lockedQuantity: number; alertThreshold: number; sku: { skuCode: string; variantName: string; product: { name: string } }; warehouse: { name: string; store: { name: string } } }
...
export default function InventoryPage() {
  ...
  const inventory = useQuery({ queryKey: ['inventory', keyword, page], queryFn: async () => (await api.get<ApiSuccess<InventoryList>>('/inventory', { params: { keyword: keyword || undefined, page, pageSize: 10 } })).data.data });
  ...
  return <div className="products-page">
    <div className="page-heading"><div><Typography.Title level={2}>库存</Typography.Title><Typography.Text type="secondary">查看 SKU 库存并记录入库、出库和调整</Typography.Text></div></div>
    ...
    <Table rowKey="id" loading={inventory.isLoading} dataSource={inventory.data?.items ?? []} pagination={pagination} columns={[
      { title: 'SKU / 商品', render: (_: unknown, item: Inventory) => <div><Typography.Text strong>{item.sku.skuCode}</Typography.Text><div className="muted">{item.sku.product.name} · {item.sku.variantName}</div></div> },
      ...
      { title: '操作', render: (_: unknown, item: Inventory) => <Button type="link" disabled={!canWrite} onClick={() => openAdjust(item)}>调整库存</Button> },
    ]} />
    <Modal title="调整库存" open={!!selected} onCancel={() => setSelected(null)} onOk={() => form.submit()} confirmLoading={adjust.isPending} okText="保存" cancelText="取消">
      {selected && <><DescriptionsLike label="SKU" value={`${selected.sku.skuCode} · ${selected.sku.product.name}`} /><DescriptionsLike label="仓库" value={`${selected.warehouse.store.name} / ${selected.warehouse.name}`} /><DescriptionsLike label="当前可用" value={String(selected.availableQuantity)} /><Form form={form} layout="vertical" onFinish={(values) => adjust.mutate(values)}><Form.Item label="操作类型" name="type" rules={[{ required: true }]}><Select options={[{ value: 'RESTOCK', label: '入库（增加）' }, { value: 'SALE', label: '出库（减少）' }, { value: 'ADJUSTMENT', label: '盘点调整（可正可负）' }]} /></Form.Item><Form.Item label="数量" name="quantity" rules={[{ required: true, message: '请输入数量' }]}><InputNumber style={{ width: '100%' }} /></Form.Item><Form.Item label="原因" name="reason"><Input.TextArea rows={2} placeholder="例如：采购入库、盘点修正" /></Form.Item></Form></>}
    </Modal>
  </div>;
}
